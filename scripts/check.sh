...
if [[ $FAST -eq 0 ]]; then
    # Фичи не должны быть обязательными: движок обязан собираться без единого
    # протокола, иначе «опционально» на деле не проверено.
    #
    # Здесь именно `test`, а не `build`, и это важно. Сборка проходила и
    # тогда, когда протоколы на самом деле оставались внутри: движок приходил
    # к демону и клиенту со своими умолчаниями, а в них все протоколы сразу.
    # Ловится это только тестом `every_registered_protocol_comes_from_an_enabled_feature`
    # — он сверяет реестр со списком включённых фич и в такой сборке обязан
    # увидеть оба пустыми.
    run "test --no-default-features" cargo test -p penguin-engine --no-default-features
