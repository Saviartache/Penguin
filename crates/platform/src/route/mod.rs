...
impl RouteGuard {
    ...
    pub fn pin_outside(&self, addresses: &[IpAddr], outside: &DefaultRoute) -> PlatformResult<()> {
        for address in addresses {
            ...
            // Сначала снять такой же, если он остался от убитого запуска.
            // Этот маршрут — единственный наш, который переживает падение
            // клиента: он идёт через физический интерфейс, а не через тоннель,
            // и вместе с погашенным адаптером не исчезает. Ядро отвечает на
            // повторную постановку отказом, а машина к тому времени могла
            // сменить сеть — и тогда в застрявшем маршруте стоит шлюз, до
            // которого больше нет пути. Снаружи это «рукопожатие не
            // завершилось», и переживает оно любую перезагрузку.
            let _ = remove_route(&route);
            self.add(route)?;
        }
        Ok(())
    }
