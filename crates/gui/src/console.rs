...
/// соседи — такие же строки, зазор выходит одинаковым сам собой, но строке во
/// всю высоту пришлось задать отступ отдельным числом, и совпасть эти два числа
/// могли только случайно — отсюда и разъехавшиеся зазоры вокруг неё.
///
/// Поэтому запас вынесен наружу, в [`SPACING`]: теперь между **любыми** двумя
/// строками консоли стоит одно и то же расстояние, и поток в этом смысле такая
/// же строка, как остальные.
const LINE: f32 = SIZE;

/// Единственный зазор консоли — между любыми двумя строками, включая поток.
///
...
pub enum Line {
    ...
    /// Поток данных: загрузка долей от нуля до единицы.
    ///
    ...
    /// поток, а блоки делит черта в заголовке раздела.
    Stream(f32),
}
    ...
    for line in lines {
        ...
        if let Some(element) = shown {
            body = match line {
                ...
                // забирает целиком.
                Line::Stream(_) => body.push(element),
...
fn cost(line: &Line) -> usize {
    match line {
        ...
        // Поток — данные, а не набранный текст: он появляется целиком.
        Line::Stream(_) => 0,
    }
}
...
/// Поток данных во всю оставшуюся высоту — виджет кита.
///
/// Своих летящих битов у консоли нет: поток — это `BitStream`, и вторая его
/// копия здесь означала бы два разных потока в одной семье приложений.
///
/// Столбики истории он сменил намеренно. Ряд отвечал на вопрос «росло или
/// падало», но неподвижный ряд из одинаковых столбиков и остановившийся ряд
/// выглядят одинаково, а окно открывают ради другого — идёт связь или встала.
/// Летящие биты отвечают на это движением, а числа за минуту и за всё время
/// стоят строками ниже.
fn stream<'a, Message: 'a>(load: f32) -> Element<'a, Message> {
    BitStream::new(load).w(Size::FILL).h(Size::FILL).into()
}
...
mod tests {
    ...
    #[test]
    fn the_stream_takes_all_the_height_left_over() {
        // Ради этого он и не строка: поток занимает всё, что не занял текст, а
        // не одну строку в знак высотой. Виджет кита по умолчанию высотой в
        // строку, поэтому высота задаётся здесь.
        let element: Element<'_, ()> = stream(0.5);
        let size = element.as_widget().size();
        assert_eq!(size.width, Length::Fill);
        assert_eq!(size.height, Length::Fill);
    }
    ...
    #[test]
    fn an_idle_stream_still_holds_its_place() {
        // Трафика нет — место всё равно за потоком: без этого текст расползся
        // бы по всей высоте окна.
        let element: Element<'_, ()> = stream(0.0);
