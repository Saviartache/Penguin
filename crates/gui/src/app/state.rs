...
/// Сколько отсчётов скорости держать в окне истории.
///
/// Само окно на экран не выходит: по нему считается шкала, с которой
/// сравнивается свежий отсчёт ([`Connection::graph_scale`]).
...
pub struct Connection {
    ...
    pub connections: u64,
    /// История скорости: окно, по которому считается шкала.
...
impl Connection {
    ...
    /// Наибольшая скорость окна истории — шкала.
    ///
    /// Без общей шкалы доля прыгала бы на каждом кадре, подстраиваясь под
    /// текущий максимум.
    pub fn graph_scale(&self) -> u64 {
        self.graph
            ...
            .max()
            // Пустая история всё равно должна на что-то делиться.
            .unwrap_or(1)
            .max(1)
    }
}
...
pub struct SplitTunnelState {
    ...
    /// Открыто ли системное окно выбора файла.
    ///
    /// Оно не наше и живёт отдельно: пока человек в нём ищет, окно программы
    /// остаётся живым, и второе нажатие кнопки открыло бы второе такое же.
    pub picking: bool,
    /// Почему окно выбора файла не открылось.
    ///
    /// Сообщение показывается в самом окне правила, а не в журнале: журнал —
    /// другая вкладка, а окно правила закрывает её собой.
...
pub struct ServersState {
    ...
    /// Идёт проверка.
    pub probing: bool,
    /// Открыт ли список протоколов.
    ///
    /// Первый шаг «Добавить сервер»: пока протокол не выбран, показывать
    /// нечего — поля формы приносит он. Отдельным признаком, а не состоянием
    /// внутри редактора: редактор существующего профиля этого шага не знает,
    /// протокол у него уже есть.
    pub picker: bool,
