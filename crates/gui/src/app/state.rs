...
pub struct State {
    ...
    /// Настройки, какими их отдал демон.
    pub config: RootConfig,
    /// Есть ли несохранённые правки.
    ///
    /// Считается сравнением с тем, что отдал демон: иначе «сохранить» пришлось
    /// бы предлагать всегда, и кнопка перестала бы что-либо значить.
    pub dirty: bool,
