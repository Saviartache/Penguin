...
/// Разбирает сообщение.
pub fn handle(app: &mut App, message: Message) -> Command<Message> {
    match message {
...
fn handle_window(app: &mut App, message: WindowMessage) -> Command<Message> {
    match message {
        ...
        WindowMessage::DragStarted => {
            app.chrome_mut().arm_drag(window::Id::MAIN);
            Command::none()
        }
        WindowMessage::CursorMoved(position) => app.chrome_mut().on_cursor_moved(position),
        WindowMessage::Minimize => window::minimize(window::Id::MAIN, true),
        WindowMessage::Close => window::close(window::Id::MAIN),
...
pub fn bootstrap() -> Command<Message> {
    Command::perform(connection::ensure_at_startup(), |ready| {
        Message::Home(HomeMessage::ServiceChecked(ready))
    })
}

/// Спрашивает у демона всё, без чего окно пусто.
pub fn request_initial_state() -> Command<Message> {
    Command::batch([request(Request::GetConfig), request(Request::Status)])
}
...
pub fn request(request: Request) -> Command<Message> {
    Command::perform(crate::ipc::client::send_or_error(request), |response| {
...
mod tests {
    ...
    fn initial_state_asks_for_config_and_status() {
        ...
        // `Command` не разбирается снаружи; проверяем хотя бы, что он не
