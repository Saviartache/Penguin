...
pub struct ProtocolSpec {
    ...
    /// Строка над формой — то, что человеку надо знать до того, как он
    /// заполнит поля, а не после.
    ///
    /// `None` у почти всех: строка, которая есть везде, не читается нигде.
    /// Заведено ради ShadowsocksR: его шифры не заверяют данные, и
    /// молчать об этом в форме, которая во всём остальном похожа на форму
    /// Shadowsocks, значило бы выдавать одно за другое.
    pub note: Option<Label>,
}
