//! Журнал работы — символьной консолью кита, без её шапки.
//!
...
//!
//! Верхней полосы у панели нет ([`uikit::widgets::LogTerminal::without_header`]).
//! Заголовок в ней повторял подпись вкладки, по которой сюда и пришли, кружки
//! «светофора» — те, что рисует шапка самого окна, а счётчик строк отвечал на
//! вопрос, которого никто не задаёт. Отнимала она при этом строку у панели,
//! открытой ровно ради строк. Осталась рабочая область: консоль от края до
//! края.
//!
...
pub fn view(state: &State) -> Element<'_, Message> {
    ...
    // открыт.
    //
    // Заголовок панель всё равно берёт: он остаётся в вызове, но не рисуется,
    // — так у неё есть имя на случай, если шапку когда-нибудь вернут.
    LogTerminal::new(strings.events, &state.connection.lines)
        .without_header()
        .build()
}
...
mod tests {
    ...
    fn the_tab_fills_the_window() {
        // Панель — окно терминала: она занимает вкладку целиком.
