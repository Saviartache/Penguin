...
impl Request {
    ...
    /// Ждёт ли ответ работы, а не памяти.
    ///
    /// Не то же самое, что [`Self::is_mutating`], и путать их дорого. Замер
    /// задержки ничего в системе не меняет, но ходит до каждого сервера и
    /// меряется секундами; обход процессов не меняет тем более, а стоит обхода
    /// всей таблицы. Состояние, настройки, разбор правил демон отвечает из
    /// памяти — там счёт на миллисекунды.
    ///
    /// По этому признаку окно выбирает, сколько ждать ответа. Один срок на всех
    /// означал бы либо оборванный замер задержки, либо минуту неподвижного
    /// окна в ответ на вопрос к зависшей службе.
