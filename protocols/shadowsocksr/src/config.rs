...
pub struct ShadowsocksrConfig {
    ...
    /// это имя уже занято — им выбирается сам протокол (`RawOutbound` в
    /// `penguin-config` вынимает его до того, как остальное попадёт
    /// сюда). Поле с тем же именем было бы просто
