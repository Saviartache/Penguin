...
impl PingwinConfig {
    ...
    /// Обход, которым направление пробует пробиться само, когда человек ничего
    /// не выбрал, а приветствие не доходит.
    ///
    /// `fake` — ложное приветствие впереди настоящего. Из четырёх приёмов
    /// именно он на живой линии и сработал там, где не сработали разрез и
    /// перестановка: оборудование разбирает первое приветствие, принимает по
    /// нему решение и настоящее уже не смотрит.
    ///
    /// Отдельная функция, а не поле настроек: это не выбор человека, а
    /// последнее средство направления (см. `PingwinOutbound::raise`).
    pub fn rescue_desync() -> PingwinResult<Desync> {
        Ok(DesyncConfig {
