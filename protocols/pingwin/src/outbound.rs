...
//!
//! Первая несущая при этом поднимается **до** всякого потока — при
//! подключении профиля ([`PingwinOutbound::warm_up`]). Без этого «подключено»
//! в окне не значило бы ничего: направление собиралось бы, ни разу не сходив
//! к серверу.
//!
...
impl PingwinOutbound {
    ...
    /// Поднимает первую несущую, ничего через неё не открывая.
    ///
    /// Зовётся при подключении профиля ([`crate::factory`]). Смысл — не в
    /// прогреве, а в честности: пока сюда не сходили, никто не знает, отвечает
    /// ли сервер вообще, а окно уже написало бы «подключено».
    ///
    /// Ошибка отсюда — та самая, которую человек и должен увидеть: не тот
    /// ключ, не тот пароль, сервер молчит, путь до него режут.
    pub async fn warm_up(&self) -> PingwinResult<()> {
        self.establish(None).await.map(|_| ())
    }
