//! Двунаправленный поток `CONNECT` поверх HTTP/2: `AsyncRead + AsyncWrite`
//! над `h2::SendStream`/`RecvStream`.
//!
//! `h2` уже разделяет запрос и ответ на `SendStream`/`RecvStream` и даёт им
//! поллинговый API, так что оборачивать асинхронные методы в отдельную
//! задачу не нужно — `poll_capacity` и `poll_data` ложатся на
//! `AsyncWrite`/`AsyncRead` напрямую. То же устройство, что у
//! `protocols/naive` (`src/stream/h2.rs`), — переписано заново, а не взято
//! оттуда: протокол не может зависеть от другого протокола (`AGENTS.md`,
//! §1.1).
...
/// Приводит ошибку `h2` к языку `AsyncRead`/`AsyncWrite`.
...
impl AsyncWrite for H2Stream {
    ...
    fn poll_shutdown(self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        // Пустой кадр `DATA` с `END_STREAM` — аналог `FIN` в TCP
        // (`PROTOCOL.md`, §5.4).
        self.get_mut()
            .send
