...
//!
//! Доходит он, впрочем, не до `supervisor`, а до читателя потока: ответ
//! сервера снимается лениво, при первом чтении ([`crate::stream`]), и к
//! этому времени поток уже отдан приложению. Отказ виден как ошибка чтения,
//! а не как отказ подключения.
